    ...
    $.easyAjax = function(options) {
                ...
                if (jqXHR.responseJSON.hasOwnProperty('errors')) {
                    for (var prop in jqXHR.responseJSON.errors) {
                        ...
                        $(jqXHR.responseJSON.errors[prop]).each(function (val, msgs) {
                            ...
                            //msg+=+msgs+"<br>";
                            if(prop=="general"){
                                showResponseMessage(msgs, "error");
                            }else {
                                var ele = $(opt.container).find("[name='" + prop + "']");
                                console.log(ele.length);
                                if(ele.length==0){
                                    ele =$(opt.container).find("[data-name='" + prop + "']");
                                }
                                ele.addClass('is-invalid');
                                if(ele.parent().find('.invalid-feedback').length == 0){

                                    ele.parent().parent().find('.invalid-feedback').show().text(msgs);
                                }else{
                                    ele.parent().find('.invalid-feedback').show().text(msgs);
                                }
        ...
        function showResponseMessage(msg, type, toastrOptions) {
            ...
            if (opt.messagePosition == "toastr") {
                ...
                var html = '<div class="alert alert-'+ typeClasses[type] +' alert-dismissible">' +
        ...
        $(opt.container).find(".invalid-feedback").text("");
