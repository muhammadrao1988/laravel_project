...
{
    public static $module = "AccountGroup";
    ...
    public static function actionButtons($data){
        $return = '<div class="btn-group" role="group">
                        <a class="btn" href="'.route('accountGroups.show', $data->id).'" title="View">
    ...
    public static function syncAccountGroup(){
        ...
        try {
            ...
            if(!empty($decodedResponse->data) && (!empty($decodedResponse->data->account_groups->page_info))) {
                $pageInfo = $decodedResponse->data->account_groups->page_info;
                ...
                if($total_count > 0){
                    for($i=1;$i<=$totalPages;$i++){
                        ...
                        query MyCoolQuery(\$paginator: Paginator) {
                         account_groups(paginator: \$paginator) {
                          entities {
                           id
                           sonar_unique_id
                           name
                        ...
                        $decodedResponse = json_decode($response->getBody()->getContents(), false, JSON_PRETTY_PRINT);
                        if(!empty($decodedResponse->data) && (!empty($decodedResponse->data->account_groups->entities))){

                            foreach ($decodedResponse->data->account_groups->entities as $data){
                                self::updateOrCreate(
                                    ['sonar_id' => $data->id],
                                    [
                                        'sonar_id' => $data->id,
                    ...
                    return ['success'=>true,'msg'=>"Account Groups  synced successfully.."];
                }else{
